...
В проекте есть готовый `scripts/serve.py`. Запускай его из корня проекта (не создавай свой сервер):
...
Сервер поднимает локальный HTTP на порту 8000, отдаёт CORS-заголовки (нужны для html2canvas) и открывает `index.html` в браузере. Если порт занят, сервер показывает PID и спрашивает подтверждение перед остановкой процесса.
...
- CSS-размер слайда остаётся 1280×720px (соотношение 16:9) — не меняется при экспорте;
- PNG рендерится в 3840×2160px за счёт `scale: 3` в html2canvas — только в момент экспорта.

---

## Разрешение экспорта

**Всегда используй `scale: 3`** в html2canvas — это 288 dpi, пригодное для печати и презентаций.
...
out.width = W * SCALE;
out.height = H * SCALE;
out.getContext('2d').drawImage(canvas, 0, 0, W * SCALE, H * SCALE, 0, 0, W * SCALE, H * SCALE);
```

`scale: 1` давало размытые PNG при масштабировании и печати. `scale: 3` — обязательный минимум для читаемого результата.
